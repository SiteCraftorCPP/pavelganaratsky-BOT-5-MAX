...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        try:
            ...
            pass
        try:
            await db.execute("ALTER TABLE users ADD COLUMN dialog_chat_id INTEGER")
        except Exception:
            pass
        await db.commit()
...
async def set_user_dialog_chat(user_id: int, chat_id: int) -> None:
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            """
            INSERT INTO users (user_id, dialog_chat_id) VALUES (?, ?)
            ON CONFLICT(user_id) DO UPDATE SET dialog_chat_id = excluded.dialog_chat_id
        """,
            (user_id, chat_id),
        )
        await db.commit()
...
async def get_user_dialog_chat(user_id: int) -> int | None:
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT dialog_chat_id FROM users WHERE user_id = ?", (user_id,)
        ) as cursor:
            row = await cursor.fetchone()
            if not row or row["dialog_chat_id"] is None:
                return None
            return int(row["dialog_chat_id"])
...
async def get_users():
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT user_id, timezone, language, dialog_chat_id FROM users"
        ) as cursor:
